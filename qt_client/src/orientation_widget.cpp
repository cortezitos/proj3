...
Vec3 rotate_point(const Quaternion & q_in, const Vec3 & v)
{
...
QPointF project_point(const Vec3 & point, const QRectF & viewport)
{
    const double camera_distance = 4.5;
    const double depth = camera_distance - point.z;
...
void OrientationWidget::paintEvent(QPaintEvent * event)
{
    ...
    {
        projected[i] = project_point(rotate_point(quaternion_, cube[i]), viewport);
    }
    ...
    painter.drawLine(viewport.center(), project_point(rotate_point(quaternion_, {1.6, 0, 0}), viewport));
    painter.setPen(QPen(QColor("#06d6a0"), 3));
    painter.drawLine(viewport.center(), project_point(rotate_point(quaternion_, {0, 1.6, 0}), viewport));
    painter.setPen(QPen(QColor("#118ab2"), 3));
    painter.drawLine(viewport.center(), project_point(rotate_point(quaternion_, {0, 0, 1.6}), viewport));
